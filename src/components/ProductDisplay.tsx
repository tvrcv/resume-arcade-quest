      ...
      <div className="grid grid-rows-2 gap-8 h-full">
        ...
        <div className="flex justify-between items-center px-4">
          {sections.slice(0, 3).map((section) => {
            const isDispensing = selectedSection?.id === section.id && machineState === 'dispensing';
            return (
              <div key={section.id} className="relative group">
                <div className={`
                  w-16 h-20 rounded-xl shadow-lg border border-white/40 overflow-hidden
                  ${isDispensing ? 'opacity-30' : 'opacity-100'}
                ...
                {/* Depth stack - Only show if not dispensing */}
                {!isDispensing && (
                  <>
